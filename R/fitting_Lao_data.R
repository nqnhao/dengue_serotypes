...
data_lao <- data_lao %>% select("Age (years)", "DENV1", "DENV2", "DENV3", "DENV4") %>% 
...
fit_Lao_2009 <- fit(data_lao)
print(fit_Lao_2009, pars = c("lambda1", "lambda2", "sigma12", "sigma21"), probs = c(0.025, 0.5, 0.975), digits = 4)


##PPC Lao_2009
age_fine <- seq(5, max(data_lao$age), by=1)
n_age_fine <- length(age_fine)
stan_data <- list(
  N = nrow(data_lao),
  age = data_lao$age,
  n_tested = data_lao$n_tested,
  n_s = data_lao$n_s,
  n_denv1 = data_lao$n_denv1,
  n_denv2 = data_lao$n_denv2,
  n_denv12 = data_lao$n_denv12,
  y = as.matrix(data_lao[, c("n_s", "n_denv1", "n_denv2", "n_denv12")]),
  age_fine = age_fine,
  n_age_fine = n_age_fine
)
...
posterior <- rstan::extract(fit_Lao_2009)
